...
if __name__ == '__main__':
    # Always DoFo
    t = DoFo(queue_to_fo, id_name_dict)
    t.daemon = True
    t.start()

    # Always DoUnfo
    t = DoUnfo(queue_to_unfo)
    t.daemon = True
    t.start()

    for cmd in sys.argv[1:]:
        if cmd == 'fo':
            t = GenFo(queue_to_fo, id_name_dict, poked)
            t.daemon = True
            t.start()
        elif cmd == 'unfo':
            t = GenUnfo(queue_to_unfo, id_name_dict)
            t.daemon = True
            t.start()
        elif cmd.startswith('steal'):
            steal_id = cmd[cmd.index('(') + 1: cmd.index(')')]
            t = StealFoers(steal_id, queue_to_fo, id_name_dict)
            t.daemon = True
            t.start()
        else:
