...
class InstagramAPI:
    ...
    def throwIfInvalidUsertags(self, usertags):
        for user_position in usertags:
            ...
            correct = True
            if isinstance(user_position, type({})):
                position = user_position.get('position', None)
                user_id = user_position.get('user_id', None)

                if isinstance(position, type([])) and len(position) == 2:
                    try:
                        x = float(position[0])
                        y = float(position[1])
                        if x < 0.0 or x > 1.0:
            ...
            if item.get('type', '') == 'video':
                ...
                videoConfig = {
                    ...
                    'edits': {
                        ...
                        'cinema': 'unsupported',
                        # round($videoDetails->getDuration(), 1),
    ...
    def direct_share(self, media_id, recipients, text=None):
        if not isinstance(recipients, type([])):
    ...
    def deleteComment(self, mediaId, commentId):
        ...
        return self.SendRequest('media/' + str(mediaId) + '/comment/' +
                                str(commentId) + '/delete/', self.generateSignature(data))
    ...
    def getHashtagFeed(self, hashtagString, maxid=''):
        return self.SendRequest('feed/tag/' + hashtagString + '/?max_id=' + str(maxid) +
                                '&rank_token=' + self.rank_token + '&ranked_content=true&')
    ...
    def getLocationFeed(self, locationId, maxid=''):
        return self.SendRequest('feed/location/' + str(locationId) + '/?max_id=' + maxid +
                                '&rank_token=' + self.rank_token + '&ranked_content=true&')
    ...
    def getUserFollowers(self, usernameId, maxid=''):
        if maxid == '':
            ...
            return self.SendRequest('friendships/' + str(usernameId) +
                                    '/followers/?rank_token=' + self.rank_token + '&max_id=' + str(maxid))
    ...
    def buildBody(self, bodies, boundary):
        ...
        for b in bodies:
            ...
            if _headers and isinstance(_headers, type([])):
                for h in _headers:
